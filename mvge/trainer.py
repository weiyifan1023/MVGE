...
result_saved_path = os.path.join(args.save_dir, result_time + "-MVG")  # 新设定path
...
def main():
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        valid_f1.append(metrics["f1"])
        valid_losses.append(model.dev_loss.avg)  # 保存当前epoch的dev loss
        model.avg_reset()
        if metrics["f1"] > best_result:
            save_prefix = os.path.join(result_saved_path, "checkpoint_best")  # model_save path: try/
