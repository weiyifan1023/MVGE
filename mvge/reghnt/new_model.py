...
from reg_hnt.reghnt.modeling_tree import TreeNode, Score, TreeAttn, Prediction, TreeEmbedding, TreeBeam, generate_tree_input, GenerateNode, \
    Merge, copy_list
...
# Multi-view Graph TAT
class MVGTAT(nn.Module):
    def __init__(self, gnn_hidden_size):
        super(MVGTAT, self).__init__()
        self.ndim = gnn_hidden_size
        self.dropout = 0.5
        self.gcn = Graph_Module(indim=gnn_hidden_size, hiddim=gnn_hidden_size, outdim=gnn_hidden_size, dropout=self.dropout)  # bert hidden size

    def forward(self, q_mean, x, batch):
        multi_graph = batch['b_multi_graph']
        _, x = self.gcn(q_mean, x, multi_graph)  # Param 1 特征矩阵 Param 2 邻接矩阵
        return x
...
class Graph_Module(nn.Module):
    def __init__(self, indim, hiddim, outdim, dropout):
        ...
        self.d_k = outdim  # 先不压缩

        self.gcn_graph = nn.ModuleList([GCN(indim, hiddim, self.d_k, dropout=0.3) for _ in range(self.h)])

        # self.gat_graph = nn.ModuleList([GAT(indim, hiddim, self.d_k, dropout=0.3) for _ in range(self.h)])
        self.guided_layer = CoarseGraphPoolingLayer(indim, outdim)  #
        self.gate_layer1 = GateModule(indim, outdim)
        self.gate_layer2 = GateModule(indim, outdim)
        self.mvg_attention = MultiGranularityAttention(indim, outdim, self.h)
        # Coarse-Fine Guided Vector: init = Q_mean_vector
        # self.q_mean = torch.mean(q_nodes, dim=1)
        self.linear_q = nn.Linear(indim, outdim)

        self.feed_forward = PositionwiseFeedForward(indim, hiddim, outdim, dropout)
        self.norm = LayerNorm(outdim)
        self.dropout = dropout
        self.linear = nn.Linear(self.d_k * 3, outdim)
    ...
    def forward(self, q_mean, graph_nodes, graph):
        '''
        ## Inputs:
        - graph_nodes (batch_size, K, in_feat_dim): input features
        ## Returns:
        ...
        if not bool(graph.numel()):  # 返回数组中元素的个数; 邻接矩阵为空则创建
            adj = self.get_adj(graph_nodes)
            adj_list = [adj, adj, adj, adj]
        else:
            adj = graph.float()
            adj_list1 = [adj[:, 0, :], adj[:, 1, :], adj[:, 2, :]]  # 多视图分组

        g_feature = tuple([l(graph_nodes, g_adj) for l, g_adj in zip(self.gcn_graph, adj_list1)])  # 按视图遍历，图是按batch中max_len构建的
        # g_feature = tuple([l(graph_nodes, adj_list1[0]) for l, g_adj in zip(self.gcn_graph, adj_list1)])  # 只用coarse graph; raltion图
        # g_feature = tuple([l(graph_nodes, g_adj) for l, g_adj in zip(self.gat_graph, adj_list1)])  # 按视图遍历，图是按batch中max_len构建的

        # Multi-view attention
        g_feature = torch.cat(g_feature, dim=2)  # B x S x KH ; k=2
        g_feature = self.mvg_attention(g_feature, q_mean, flag=0)
        """
        # Guided && gate layer
        
        q_c = self.linear_q(q_mean)
        graph_nodes = self.gate_layer1(graph_nodes, q_c)
        g_coarse = self.gcn_graph[0](graph_nodes, adj_list1[2])

        command_vector = self.guided_layer(g_coarse, adj_list1[2], q_c)
        g_coarse = self.gate_layer2(g_coarse, command_vector)  # 防止偏离Q, 决定接受多少Q信息，以更好地指导相关node的重要性分配
        g_fine = self.gcn_graph[1](g_coarse, adj_list1[1])
        g_fine = self.gcn_graph[2](g_fine, adj_list1[0])  # B X S X H

        # multi-granularity layer
        
        # new_g_coarse = torch.flatten(g_coarse, start_dim=1).unsqueeze(dim=1)  # B X (S*H)
        # new_g_fine = torch.flatten(g_fine, start_dim=1).unsqueeze(dim=1)  # B X (S*H)
        #  c_i concatenation k-view representations of node v_i (k=2 两个层次结构)
        g_col = torch.cat((g_coarse, g_fine), dim=2)  # B x S x KH ; k=2
        g_final = self.mvg_attention(g_col, q_mean)
        g_feature = g_final
        """

        # graph transformer部分
        # g_feature = torch.cat(g_feature, dim=2)
        # g_feature = self.linear(F.dropout(g_feature, self.dropout, training=self.training))

        g_feature = self.norm(g_feature) + graph_nodes

        graph_encode_features = self.feed_forward(g_feature) + g_feature

        return adj, graph_encode_features  # first not use
...
class GCN(nn.Module):
    ...
    def forward(self, x, adj):
        '''
        x=graph_nodes: (K=S, in_feat_dim=hidden_size)
        S = ∑ sen_bs_i {q+t+p}
        '''
...
class GraphConvolution(Module):
    ...
    def forward(self, input, adj):
        '''
        input=graph_nodes: (bs, sen, hidden_size)
        '''
...
class GAT(nn.Module):
    def __init__(self, nfeat, nhid, nout, dropout=0.2, alpha=0.2, nheads=3):
    ...
    def forward(self, x, adj):
        x = F.dropout(x, self.dropout, training=self.training)
        x = torch.cat([att(x, adj) for att in self.attentions], dim=2)  # 已经激活
        x = F.dropout(x, self.dropout, training=self.training)
...
class new_GraphAttentionLayer(nn.Module):
    ...
    def forward(self, X, adj):
        ...
        # [B, N, N, 1] => [B, N, N] attention (not scaler) 图注意力的相关系数（未归一化）
        ...
        # [B, N, N] * [B, N, out_features] => [B, N, out_features]  最后两个维度做矩阵乘法
...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        self.a = nn.Parameter(torch.empty(size=(2 * out_features, 1)))  # attention kernel
        nn.init.xavier_uniform_(self.a.data, gain=1.414)

        self.leakyrelu = nn.LeakyReLU(self.alpha)

    def forward(self, h, adj):
        bs = h.size(0)  # 用h而不用x ，其想表达的是这个节点特征会随着层的堆叠而改变，所以用h来表示隐藏层特征hidden feature
        N = h.size(1)  # nodes的个数
        Wh = torch.matmul(h, self.W)  # h.shape: (B, N, in_features), Wh.shape: (B, N, out_features)  mm -> matmul

        e = self._prepare_attentional_mechanism_input(Wh)

        zero_vec = -9e15 * torch.ones_like(e)  # 负无穷,填充无连接位置
        attention = torch.where(adj > 0, e, zero_vec)  # adj邻接矩阵做mask,
        attention = F.softmax(attention, dim=2)  # 输出注意力系数矩阵  B * N * N
        attention = F.dropout(attention, self.dropout, training=self.training)
        h_prime = torch.matmul(attention, Wh)  # 表示这层GAL关于节点h的输出特征
    ...
    def _prepare_attentional_mechanism_input(self, Wh):
        ...
        # Wh1&2.shape (N, 1)
        # e.shape (N, N)
        Wh1 = torch.matmul(Wh, self.a[:self.out_features, :])  # kernel 第一部分是 attn_for_self  [a_1]^T [Wh_i]
        Wh2 = torch.matmul(Wh, self.a[self.out_features:, :])  # kernel 第二部分是 attn_for_neighs [a_2]^T [Wh_j]
        # broadcast add
        # e = Wh1 + Wh2.T
        e = Wh1 + Wh2.permute(0, 2, 1)  # 注意力系数矩阵前身
...
class GateModule(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.W = nn.Parameter(torch.empty(size=(in_features, out_features)))
        self.W_c = nn.Parameter(torch.empty(size=(in_features, out_features)))
        nn.init.xavier_uniform_(self.W.data, gain=1.414)
    ...
    def forward(self, h, c_h):
        x = torch.matmul(h, self.W) + torch.matmul(c_h, self.W_c)
        x = self.layernorm(x)  # add BN
        x = self.dropout(self.sigmoid(x))  # 新加dropout
        x = torch.mul(x, h) + torch.mul(1 - x, c_h)  # h与gate vector 点乘
        return x


class CoarseGraphPoolingLayer(nn.Module):
    def __init__(self, in_features, out_features):
        super(CoarseGraphPoolingLayer, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
    ...
    def forward(self, h, adj, c_h):
        N = h.size(1)  # 邻接矩阵中 nodes 的个数
        h = torch.cat((h, c_h), dim=1)  # N = sen + 1 ; c_h: command vector

        Wh = torch.matmul(h, self.W1)  # h.shape: (B, N+1, in_features), Wh.shape: (B, N+1, out_features)
        attention = self._prepare_attentional_mechanism_input(Wh)  # e.shape: (B, N+1, N+1)

        zero_vec = -9e15 * torch.ones_like(attention)  # 负无穷,填充无连接位置
        mask_adj = F.pad(adj, (0, 1, 0, 1, 0, 0), 'constant', 1).bool()  # 新的node m_q 扩充
        attention = attention.masked_scatter_(mask=~mask_adj, source=zero_vec)
        attention = attention[:, -1, :].unsqueeze(1)
        attention = F.softmax(attention, dim=2)  # 输出注意力系数矩阵  # (B,1,N+1)
        attention = F.dropout(attention, self.dropout, training=self.training)
        command_vector = torch.matmul(attention, Wh)  # pooling graph representation

        return command_vector  # new command vector: coarse to fine process
...
# 在不同粒度图中 node_i 对Q的贡献
class MultiGranularityAttention(nn.Module):
    def __init__(self, in_features, out_features, k_view=2):
        super(MultiGranularityAttention, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.k_view = k_view
        self.W_q = nn.Parameter(torch.empty(size=(in_features, out_features * self.k_view)))
        nn.init.xavier_uniform_(self.W_q.data, gain=1.414)
        self.atts = nn.ParameterList([nn.Parameter(torch.empty(size=(out_features * k_view, 1))) for _ in range(k_view)])
        for i in range(self.k_view):
            nn.init.xavier_uniform_(self.atts[i].data, gain=1.414)  # 初始化Parameter list

    def forward(self, c, q_m, flag=True):
        """
            # q_m size = B x 1 x H
            # c size = B X S X KH
            # a size = KH x 1
        ...
        K = self.k_view
        H = q_m.size(-1)
        q_c = torch.matmul(q_m, self.W_q)  # B X 1 X KH
        atts = tuple([torch.mul(atts, q_c.transpose(1, 2)) if flag else atts for atts in self.atts])  # B X KH X 1  共K个

        e = tuple([torch.matmul(c, a_k) for a_k in atts])  # B x S x 1  共K个
        betas = F.softmax(F.dropout(torch.cat(e, dim=2), 0.5, training=self.training), dim=2)  # 输出注意力系数矩阵  B x S x K
        betas = betas.unsqueeze(dim=2)  # B x S x 1 x K

        # betas = betas.view(-1, 1, K)
        # c = c.view(-1, K, H)
        c = c.view(B, S, -1, H)  # B x S x K x H
        c = torch.matmul(betas, c)  # B x S x 1 x H
        c = c.squeeze()  # B x S x H
        return c  
        

class SubwordAggregation(nn.Module):
    """ Map subword or word pieces into one fixed size vector based on aggregation method
    """
    ...
    def forward(self, inputs, batch):
        ...
        word_subword_mask = batch['word_subword_mask'].bool()  # 原先word_mask
        number_subword_mask = batch['number_subword_mask'].bool()
        word_word_lens = batch['word_word_lens']  # 原先word_subword_lens
        number_word_lens = batch['number_word_lens']  # 原先number_subword_lens

        word_word_num_X_max_word_len_mask = batch['word_word_num_X_max_word_len_mask']  # 原先word_subword_mask
        number_word_num_X_max_word_len_mask = batch['number_word_num_X_max_word_len_mask']  # 原先number_subword_mask
        # bs * 512 * hidden_size  -> word word_num or number word num * max word_len * hidden_size
        old_word, old_number = inputs.masked_select(word_subword_mask.unsqueeze(-1)), inputs.masked_select(
            number_subword_mask.unsqueeze(-1))
        words = old_word.new_zeros(word_word_lens.size(0), max(batch['word_word_lens']).item(),
                                   self.hidden_size)  # word word_num * max word_len * hidden_size

        try:
            words = words.masked_scatter_(word_word_num_X_max_word_len_mask.unsqueeze(-1),
                                          old_word)  # mask size = word_num * max word_len  扩大一维 可以广播
        except:
            import pdb;
            pdb.set_trace()

        numbers = old_number.new_zeros(number_word_lens.size(0), max(batch['number_word_lens']).item(), self.hidden_size)
        try:
            numbers = numbers.masked_scatter_(number_word_num_X_max_word_len_mask.unsqueeze(-1), old_number)
        except:
            import pdb;
            pdb.set_trace()

        words = self.word_aggregation(words, mask=word_word_num_X_max_word_len_mask)  # word word_num * hidden_size;word-len维度的sub word融合
        numbers = self.word_aggregation(numbers, mask=number_word_num_X_max_word_len_mask)

        b_word_word_mask = batch['b_word_word_mask']  # length = (number + word) word num
        b_number_word_mask = batch['b_number_word_mask']
        new_inputs = torch.zeros([b_word_word_mask.size(0), self.hidden_size]).to(device)  # (number + word)word_num * hidden_size
        new_inputs = new_inputs.masked_scatter_(b_word_word_mask.unsqueeze(-1), words)
        new_inputs = new_inputs.masked_scatter_(b_number_word_mask.unsqueeze(-1), numbers)  # (number + word)word_num * hidden_size

        inputs = torch.zeros([bs, batch['pl_mask']['b'].size(1), self.hidden_size]).to(device)  # bs * max batch total word token number * H
        inputs = inputs.masked_scatter_(batch['pl_mask']['b'].unsqueeze(-1), new_inputs)

        old_questions, old_tables, old_paragraphs = \
            inputs.masked_select(batch['pl_mask']['q'].unsqueeze(-1)), inputs.masked_select(batch['pl_mask']['t'].unsqueeze(-1)), \
            inputs.masked_select(batch['pl_mask']['p'].unsqueeze(-1))  # 获得Q T P word token level embedding

        questions = old_questions.new_zeros(bs, batch['max_len']['question'], self.hidden_size)  # 0初始化  bs x max Q word token num x H
        questions = questions.masked_scatter_(batch['pl_mask']['question'].unsqueeze(-1), old_questions)  # padding前的token来填充
        ...
        return questions, tables, paragraphs  # bs x max (Q or T or P) word token num x H
...
class InputRNNLayer(nn.Module):
    def __init__(self, input_size, hidden_size, cell='lstm', schema_aggregation='attentive-pooling', share_lstm=False):
        ...
        self.question_lstm = getattr(nn, self.cell)(self.input_size, self.hidden_size // 2, num_layers=1, bidirectional=True,
                                                    batch_first=True)
        self.table_lstm = getattr(nn, self.cell)(self.input_size, self.hidden_size // 2, num_layers=1, bidirectional=True, batch_first=True)
        self.paragraph_lstm = getattr(nn, self.cell)(self.input_size, self.hidden_size // 2, num_layers=1, bidirectional=True,
                                                     batch_first=True)
        self.schema_aggregation = schema_aggregation
        if self.schema_aggregation != 'head+tail':  # pooling head node 维度为hidden_size
            self.aggregation = PoolingFunction(hidden_size=self.hidden_size, output_size=self.hidden_size, method=schema_aggregation)
    ...
    def forward(self, input_dict, batch):
        ...
        bs = input_dict['question'].size(0)
        # ↓↓↓↓↓↓  bs x max (Q or T or P) word token num x H
        ...
        # bs * total word token num * H
        # question_nodes, paragraphs_nodes = questions, paragraphs
        # graph_nodes = torch.cat((questions, old_tables, paragraphs), dim=1)  # word level

        # ↓↓↓↓↓↓  (Q or T or P) real nodes number * H
        ...
        t_w_t_n_mask = batch['t_w_t_n_mask']  # cell num X max cell contains word tokens num
        new_tables = torch.zeros([t_w_t_n_mask.size(0), t_w_t_n_mask.size(1), self.hidden_size]).to(device)
        new_tables = new_tables.masked_scatter_(t_w_t_n_mask.unsqueeze(-1), old_tables)  # cell num * max cell contains word tokens num * H
        tables = self.aggregation(new_tables, mask=t_w_t_n_mask)  # (word -> cell) embedding ;  (total batch real) cell num * H

        # print(t_w_t_n_mask.size(0))
        # print(torch.count_nonzero(batch["pl_mask"]["row_col"]))

        row_col_node = torch.zeros([bs, batch["pl_mask"]["row_col"].size(1), batch["pl_mask"]["row_col"].size(2), self.hidden_size]).to(device)
        row_col_node = row_col_node.masked_scatter_(batch["pl_mask"]["row_col"].unsqueeze(-1), tables)  # bs * max R * max C * H
        row_nodes = torch.mean(row_col_node, dim=2)
        col_nodes = torch.mean(row_col_node, dim=1)
        row_nodes = row_nodes.contiguous().view(-1, self.hidden_size)[lens2mask(batch['lens']['row']).contiguous().view(-1)]
        col_nodes = col_nodes.contiguous().view(-1, self.hidden_size)[lens2mask(batch['lens']['col']).contiguous().view(-1)]


        graph_nodes = torch.zeros([bs, batch["mask"]["b"].size(1), self.hidden_size]).to(device)  # bs * max batch nodes num * H
        graph_nodes = graph_nodes.masked_scatter_(batch["mask"]["question"].unsqueeze(-1), questions)
        graph_nodes = graph_nodes.masked_scatter_(batch["mask"]["table"].unsqueeze(-1), tables)
        graph_nodes = graph_nodes.masked_scatter_(batch["mask"]["paragraph"].unsqueeze(-1), paragraphs)
        graph_nodes = graph_nodes.masked_scatter_(batch["mask"]["row"].unsqueeze(-1), row_nodes)
        graph_nodes = graph_nodes.masked_scatter_(batch["mask"]["col"].unsqueeze(-1), col_nodes)

        # graph_node 特征矩阵
        # hybrid_nodes = torch.cat((questions, tables, paragraphs), dim=0)
        # cell_node_tables = torch.zeros([bs, batch["max_len"]["table"], self.hidden_size]).to(device)  # bs * max cell nodes num * H
        # cell_node_tables = cell_node_tables.masked_scatter_(batch["pl_mask"]["cell"].unsqueeze(-1), tables)  # bs * batch max cell node num * H
        # graph_nodes = torch.cat((question_nodes, cell_node_tables, paragraph_nodes), dim=1)

        questions = questions.split(batch['lens']['question'].tolist(), dim=0)
        tables = tables.split(batch['lens']['table'].tolist(), dim=0)  # 按照cell num这个维度去分，每大块包含 real T cell  num个小块
        paragraphs = paragraphs.split(batch['lens']['paragraph'].tolist(), dim=0)
        # Q T P nodes
        # output_lengths = [len(th) for q_t_c in zip(questions, tables, paragraphs) for th in q_t_c]  # sen_len list
        outputs = [th for q_t_c in zip(questions, tables, paragraphs) for th in q_t_c]  # q，t，p total_S_len
        outputs = torch.cat(outputs, dim=0)

        return outputs, graph_nodes  # total_sen * hidden_size  total_sen = for i in range(bs): total_sen += q_t_c_sen_i
...
class Arithmetic(nn.Module):
    ...
    def forward(self, sequence_output, hidden, batch, arithmetic_labels):
        ...
        all_nums_encoder_outputs, indices, masked_index = self.get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size,
                                                                                              max_num_size, self.hidden_size)
        ...
        for t in range(max_target_length):
            ...
            num_score, op, current_embeddings, current_context, current_nums_embeddings, current_attn = self.prediction(
                node_stacks, left_childs, encoder_outputs, all_nums_encoder_outputs, padding_hidden, seq_mask, num_mask, indices,
                masked_index, question_mask)
    ...
    def predict(self, sequence_output, hidden, batch, arithmetic_labels, predicted_scale_class, gold_answer):
        ...
        numbers = OPERATOR_LIST + CONST_LIST2 + batch['question_number'][arithmetic_labels.index(1)] + batch['table_number'][
            arithmetic_labels.index(1)] + batch['paragraph_number'][arithmetic_labels.index(1)]
        question_id = batch['question_uid'][arithmetic_labels.index(1)]

        numbers_scale = [-1 for _ in range(len(OPERATOR_LIST) + len(CONST_LIST2))] + batch['t_scale'][0].tolist() + batch['p_scale'][
            0].tolist()
        ...
        if len(predict_answer_id) == 0 or predict_answer_id is None or zero_count == len(predict_answer_id):
            ...
            if len(predict_answer_id) == 1 and predict_answer_id[0] >= len(OPERATOR_LIST) + len(CONST_LIST2) + len(
                    batch['question_number'][arithmetic_labels.index(1)]) and predict_answer_id[0] < len(OPERATOR_LIST) + len(
                CONST_LIST2) + len(batch['question_number'][arithmetic_labels.index(1)]) + len(
                batch['table_number'][arithmetic_labels.index(1)]):
                answer_id = predict_answer_id[0] - len(OPERATOR_LIST) - len(CONST_LIST2) - len(
                    batch['question_number'][arithmetic_labels.index(1)])
...
class OtherModel(nn.Module):
    ...
    def forward(self, sequence_output, hidden, batch, other_labels):
        ...
        tags = batch['word_tokens_tags'].masked_select(torch.tensor(other_labels).unsqueeze(-1).bool()).view(batch_size, -1).float()
    ...
    def forward2(self, sequence_output, hidden, batch, other_labels):
        ...
        question_mask = batch['mask']['question'].masked_select(torch.tensor(other_labels).unsqueeze(-1).bool()).view(batch_size, -1).long()
        table_mask = batch['mask']['table'].masked_select(torch.tensor(other_labels).unsqueeze(-1).bool()).view(batch_size, -1).long()
        paragraph_mask = batch['mask']['paragraph'].masked_select(torch.tensor(other_labels).unsqueeze(-1).bool()).view(batch_size,
                                                                                                                        -1).long()
