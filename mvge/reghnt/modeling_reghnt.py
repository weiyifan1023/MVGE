    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            for i in range(len(predicted_operator_class)):
                ...
                if batch_size == 1:
                    pass
                if "which year " in batch['questions'][i] or "Which year " in batch['questions'][i] or "Which FY " in batch['questions'][i] or "which FY " in batch['questions'][i]:
                    if isinstance(answer, list) and len(answer) == 1 and is_year(answer[0]):
                        answer[0] = clean_year(answer[0])

                if "which years" in batch['questions'][i] or "Which years" in batch['questions'][i]:
                    if isinstance(answer, list) and len(answer) > 1:
                        for j in range(len(answer)):
                            if is_year(answer[j]):
                                answer[j] = clean_year(answer[j])
