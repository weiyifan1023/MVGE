...
class TagopPredictModel():
    ...
    @torch.no_grad()
    def predict2(self, test_data_list, input_dict, step):
        test_data_list.reset()
        self.network.eval()
        pred_json = {}
        for batch in tqdm(test_data_list):
            ...
            if step == "op":
                for i in range(len(question_id)):
                    pred_json[question_id[i]] = [output_dict["operator"]]

        return pred_json
    @torch.no_grad()
    def predict_op(self, test_data_list):
        test_data_list.reset()
        self.network.eval()
        pred_json = {}
        for batch in tqdm(test_data_list):
    ...
    @torch.no_grad()
    def predict_scale(self, test_data_list, scale_predict_dict):
        test_data_list.reset()
        self.network.eval()
        for batch in tqdm(test_data_list):
            output_dict = self.network.predict2(**batch, mode="eval")
    ...
    @torch.no_grad()
    def predict_span(self, test_data_list, answer_predict_dict):
        test_data_list.reset()
        self.network.eval()
        for batch in tqdm(test_data_list):
            op_scale = answer_predict_dict[batch['question_id'][0]][0:2]
            ...
            pred_scale = output_dict["scale"]
            for i in range(len(question_id)):
                if answer_predict_dict[question_id[i]][0] == 0:
    ...
    @torch.no_grad()
    def predict_multi(self, test_data_list, answer_predict_dict):
        test_data_list.reset()
        self.network.eval()
        for batch in tqdm(test_data_list):
            op_scale = answer_predict_dict[batch['question_id'][0]][0:2]
            ...
            pred_scale = output_dict["scale"]
            for i in range(len(question_id)):
                if answer_predict_dict[question_id[i]][0] == 1:
                    if len(answer_predict_dict[question_id[i]]) == 2:
                        answer_predict_dict[question_id[i]].append([pred_answer[i], pred_scale[i]])
                    else:
                        answer_predict_dict[question_id[i]][2] =[pred_answer[i], pred_scale[i]]
    ...
    @torch.no_grad()
    def predict_count(self, test_data_list, answer_predict_dict):
        test_data_list.reset()
        self.network.eval()
        for batch in tqdm(test_data_list):
            op_scale = answer_predict_dict[batch['question_id'][0]][0:2]
    ...
    @torch.no_grad()
    def predict_arithmetic(self, test_data_list, answer_predict_dict):
        test_data_list.reset()
        self.network.eval()
        for batch in tqdm(test_data_list):
            op_scale = answer_predict_dict[batch['question_id'][0]][0:2]
