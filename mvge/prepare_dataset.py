...
parser.add_argument("--input_path", type=str, default='../dataset_reghnt')  # 服务器 ./  本地../
...
for dm in data_mode:
    ...
    print("Save data to {}.".format(os.path.join(args.output_dir, f"coarse_wyf_reghnt_{args.encoder}_cached_{dm}.pkl")))
    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)
    # with open(os.path.join(args.output_dir, f"reghnt_{args.encoder}_cached_{dm}.pkl"), "wb") as f:  # 原先
    with open(os.path.join(args.output_dir, f"coarse_wyf_reghnt_{args.encoder}_cached_{dm}.pkl"), "wb") as f:
        pickle.dump(data, f)
