...
def main():
    ...
    # # network.load_state_dict(torch.load(os.path.join(args.model_path, "checkpoint_fine-tune_0.pt")))  # fine-tuing
    # model = RegHNTPredictModel(args, network)
    #
    # logger.info("Below are the result on {} set...".format(args.mode))
    # model.reset()
    # model.avg_reset()
    #
    # if args.mode == 'dev':
    ...
    # elif args.mode == 'test':
    #     pred_json = model.predict(dev_itr, compute=False)  # 不计算F1
    #     json.dump(pred_json, open(os.path.join(args.save_dir, 'pred_result_on_test.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
    if args.mode == 'dev':
        compute = True
    else:
        compute = False
    model_path = os.path.join(args.model_path, "12231654-MVG-ensemble-best")
    save_dir = os.path.join(args.model_path, "12231654-MVG-ensemble-best")
    order_execute = ["op", "scale", "span", "arithmetic", "multi", "count"]
    for step in order_execute:
        args.step = step
        if args.step == 'op':
            network.load_state_dict(torch.load(os.path.join(model_path, "best_op_checkpoint.pt")))
            model = RegHNTPredictModel(args, network)
            model.reset()
            model.avg_reset()
            pred_json = model.predict_op(dev_itr)
            json.dump(pred_json, open(os.path.join(save_dir, f'{args.mode}_op_step.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
        elif args.step == 'scale':
            network.load_state_dict(torch.load(os.path.join(model_path, "best_scale_checkpoint.pt")))
            model = RegHNTPredictModel(args, network)
            model.reset()
            model.avg_reset()
            op_predict_dict = json.load(open(os.path.join(save_dir, f'{args.mode}_op_step.json'), 'r'))
            pred_json = model.predict_scale(dev_itr, op_predict_dict)
            json.dump(pred_json, open(os.path.join(save_dir, f'{args.mode}_scale_step.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
        elif args.step == 'span':
            network.load_state_dict(torch.load(os.path.join(model_path, "best_span_checkpoint.pt")))
            model = RegHNTPredictModel(args, network)
            model.reset()
            model.avg_reset()
            span_predict_dict = json.load(open(os.path.join(save_dir, f'{args.mode}_scale_step.json'), 'r'))
            pred_json = model.predict_span(dev_itr, span_predict_dict)
            json.dump(pred_json, open(os.path.join(save_dir, f'{args.mode}_new_step.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
        elif args.step == 'arithmetic':
            network.load_state_dict(torch.load(os.path.join(model_path, "best_arithmetic_checkpoint.pt")))
            model = RegHNTPredictModel(args, network)
            model.reset()
            model.avg_reset()
            span_predict_dict = json.load(open(os.path.join(save_dir, f'{args.mode}_new_step.json'), 'r'))
            pred_json = model.predict_arithmetic(dev_itr, span_predict_dict)
            json.dump(pred_json, open(os.path.join(save_dir, f'{args.mode}_new_step.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
        elif args.step == 'multi':
            network.load_state_dict(torch.load(os.path.join(model_path, "best_multi_checkpoint.pt")))
            model = RegHNTPredictModel(args, network)
            model.reset()
            model.avg_reset()
            span_predict_dict = json.load(open(os.path.join(save_dir, f'{args.mode}_new_step.json'), 'r'))
            pred_json = model.predict_multi(dev_itr, span_predict_dict)
            json.dump(pred_json, open(os.path.join(save_dir, f'{args.mode}_new_step.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
        elif args.step == 'count':
            network.load_state_dict(torch.load(os.path.join(model_path, "best_count_checkpoint.pt")))
            model = RegHNTPredictModel(args, network)
            model.reset()
            model.avg_reset()
            span_predict_dict = json.load(open(os.path.join(save_dir, f'{args.mode}_new_step.json'), 'r'))
            pred_json = model.predict_count(dev_itr, span_predict_dict)
            json.dump(pred_json, open(os.path.join(save_dir, f'pred_result_on_gold2_{args.mode}.json'), 'w'), ensure_ascii=False, cls=NpEncoder)
